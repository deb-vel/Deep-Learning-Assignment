...
class Model(object):
    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            dropout_prob = tf.cond(self.dropout, lambda: tf.constant(1.0 - dropoutRate, tf.float32), lambda: tf.constant(1.0, tf.float32))
            self.params = []
            ...
            with tf.variable_scope('hidden'):
                W = tf.get_variable('W', [input_size, hiddenLayerSize], tf.float32,
                                    tf.random_normal_initializer(stddev=init_stddev))
                b = tf.get_variable('b', [hiddenLayerSize], tf.float32, tf.zeros_initializer())
                self.params.extend([W, b])
                logits = tf.matmul(self.images, W) + b
                self.hs = tf.nn.dropout(tf.sigmoid(logits), dropout_prob)

            with tf.variable_scope('output'):
                W = tf.get_variable('W', [hiddenLayerSize, outputSize], tf.float32,
                                    tf.random_normal_initializer(stddev=init_stddev))
                b = tf.get_variable('b', [outputSize], tf.float32, tf.zeros_initializer())
                self.params.extend([W, b])
            ...
            self.error = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.targets, logits=tf.matmul(self.hs, W) + b))

            self.optimiser_step = tf.train.GradientDescentOptimizer(learningRate).minimize(self.error)
    ...
    def optimisation_step(self, images, targets):
        return self.sess.run([self.optimiser_step], {self.images: images, self.targets: targets, self.dropout: True})

    def get_params(self):
        return self.sess.run(self.params, {})

    def get_error(self, images, targets):
        return self.sess.run([self.error], {self.images: images, self.targets: targets, self.dropout: False})[0]

    def get_sensitivity(self, image):
        return self.sess.run([self.prediction, self.sensitivity], {self.images: [image], self.dropout:False})

    def predict(self, images):
        return self.sess.run([self.probs], {self.images: images, self.dropout: False})[0]
...
for row in range(2):
    for col in range(5):
        ...
        axs[row, col].text(1.0, 0.0, str(prediction),
                           dict(fontsize=10, ha='right', va='top', transform=axs[row, col].transAxes),
                           color='red' if prediction != digit else 'green')
...
confusion_matrix = [[np.sum(predictions[test_lbls == target] == output) for output in range(10)] for target in
                    range(10)]
